...
const facadeAndValidate = async (data, runConsistencyCheck) => {
  await runConsistencyCheck(() => {
    validate(
      getEstimatedSlotTimestamp(data.epoch, data.slot) ===
        moment(data.time).unix(),
...
export const fetchBlockBySlot = async (
  { elastic, E, runConsistencyCheck },
  { epoch, slot }
) => {
  assert(epoch != null);
...
export const fetchPreviousBlock = async (
  { elastic, E, runConsistencyCheck },
  { epoch, slot }
) => {
  assert(epoch != null);
...
// TODO: extract repetitive code
export const fetchNextBlock = async (
  { elastic, E, runConsistencyCheck },
  { epoch, slot }
) => {
  assert(epoch != null);
