...
export const parseAdaValue = (value: RawAdaValue): BigNumber => {
  ...
  if (res.abs().lt(9e15)) {
    ...
    validate(
      res.eq(resApprox),
      "parseAdaValue: ADA value should be exact at this magnitude",
      value
    );
  } else {
...
export const getEstimatedSlotTimestamp = (epoch: number, slot: number) => {
  ...
  const startTs =
    GENESIS_UNIX_TIMESTAMP_SEC +
    slot * SLOT_DURATION_SEC +
...
const getEpochSlotFromTimestamp = (timestamp: moment) => {
  const diff =
    moment(timestamp).unix() - moment.unix(GENESIS_UNIX_TIMESTAMP_SEC).unix();
  assert(diff >= 0);
...
export const calculateAge = (createdAt: moment, currentEpoch: number) => {
  ...
  const epochCount =
    epochsDiff >= 0 && epochsDiff <= 1
      ? 0
      : epochsDiff - SNAPSHOT_DURATION_IN_EPOCHS;
  return epochCount;
};
