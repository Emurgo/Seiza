...
const filterData = (data, searchText, performance) => {
  const _filtered = searchText
    ? data.filter(pool => pool.name.includes(searchText))
    : data;
  return performance
...
const sortData = (data, sortBy) =>
  _.orderBy(data, d => d.summary[sortBy], "desc");

const getFilteredAndSortedPoolsData = (
  api,
  sortBy,
  searchText,
  performance
...
export const pagedStakePoolListResolver = (
  ...
  const data = getFilteredAndSortedPoolsData(
    api,
    sortBy,
    searchText,
    performance
  ...
  const nextCursor =
    nextCursorPosition < data.length ? data[nextCursorPosition].poolHash : null;
...
const ageResolver = async (pool, args, context) => {
  const currentEpoch = await currentStatusResolver(
    null,
    args,
    context
  ).epochNumber();
...
export default {
  ...
  StakePoolSummary: {
    ...
    usersAdaStaked: stakepoolSummary => {
      return new BigNumber(stakepoolSummary.adaStaked).minus(
        stakepoolSummary.ownerPledge.declared
      );
    }
  },
  ...
  Query: {
    ...
    stakePools: (root, args, context) =>
      args.poolHashes.map(poolHash =>
        fetchBootstrapEraPool(null, poolHash, args.epochNumber)
      ),
    stakePoolList: (root, args, context) =>
      fetchBootstrapEraPoolList(null, args.epochNumber),
