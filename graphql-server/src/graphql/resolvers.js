import {
  pagedBlocksResolver,
  pagedBlocksInEpochResolver
} from "./block/resolvers";
...
import {
  fetchTransaction,
  fetchTransactionsOnAddress
} from "./transaction/dataProviders";
import {
  fetchBootstrapEraPool,
  fetchBootstrapEraPoolSummary
} from "./stakepool/dataProviders";
...
const _resolvers = {
  ...
  Query: {
    transaction: (root, args, context) =>
      fetchTransaction(context, args.txHash),
    ...
    slot: (root, args, context) =>
      fetchBlockBySlot(context, { epoch: args.epoch, slot: args.slot }),
    blockChainSearch: blockChainSearchResolver
  },
  Block: {
    isEmpty: block => block.blockHash == null,
    transactions: (block, args, context) =>
      block._txs.map(id => fetchTransaction(context, id)),
    blockLeader: (block, args, context) =>
      fetchBootstrapEraPool(null, block._blockLeader),
  ...
  Mutation: {
    subscribeToNewsletter: (root, args, context) =>
      subscribe(context, args.email)
  }
};
